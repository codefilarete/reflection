 ...
 * @see Accessors
 */
public class ReadWriteAccessPoint<C, T> implements ReversibleAccessor<C, T>, ReversibleMutator<C, T> {
	 ...
	 * @return a {@link ReadWriteAccessPoint} that will access a property through the given getter and getter
	 */
	public static <C, T> ReadWriteAccessPoint<C, T> fromMethodReference(SerializableAccessor<C, T> accessor, SerializableMutator<C, T> mutator) {
		return new ReadWriteAccessPoint<>(new AccessorByMethodReference<>(accessor), new MutatorByMethodReference<>(mutator));
	}
	
	private final Accessor<C, T> accessor;
	private final Mutator<C, T> mutator;
	
	public ReadWriteAccessPoint(ReversibleAccessor<C, T> accessor) {
		this(accessor, accessor.toMutator());
	}
	
	public ReadWriteAccessPoint(ReversibleMutator<C, T> mutator) {
		this(mutator.toAccessor(), mutator);
	}
	
	public ReadWriteAccessPoint(Accessor<C, T> accessor, Mutator<C, T> mutator) {
	...
	public boolean equals(Object obj) {
		if (this == obj) {
			return true;
		} else if (!(obj instanceof ReadWriteAccessPoint)) {
			return super.equals(obj);
		} else {
			return Predicates.equalOrNull(this.getAccessor(), ((ReadWriteAccessPoint) obj).getAccessor())
					&& Predicates.equalOrNull(this.getMutator(), ((ReadWriteAccessPoint) obj).getMutator());
		}
	}
